/* NAME : ABHIJEET ANAND
    ROLL : 19103001
    SEMESTER : IV
    Implement the Kruskal's Algorithm for the given graphs being attached Check the output Screen.
*/
...
int main()
{
      ...
      make_a_graph();
      create_tree(tree);
      printf("Edges to be included in Minimum Spanning Tree:\n");
      for(count = 1; count <= vertices - 1; count++)
      {
            printf("Edge %d->", tree[count].x);
            printf("%d\n", tree[count].y);
            tree_weight = tree_weight + tree[count].weight;
      }
      printf("cost of this Minimum Spanning Tree: %d\n", tree_weight);
      return 0;
}
...
void create_tree(struct edge tree[])
{
      ...
      if(count < vertices - 1)
      {
            printf("Spanning tree doesn't exists as it doesn't satisfy the properties of spanning tree.\n");
            exit(1);
      }
}
...
void make_a_graph()
{
      ...
      for(count = 0; count < maximum_edges; count++)
      {
            ...
            printf("Enter starting Point/vertex:");
            scanf("%d", &origin_vertex);
            printf("Enter Destination Point/vertex:");
